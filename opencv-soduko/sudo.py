...
class Sudo():
    ...
    def _check_same_num(self):
        for b_r, b_c in self.base_points:
            ...
            for i in range(1, 10):
                result = [0 if not isinstance(x[1], list) else x[0] + 1 if x[1].count(i) else 0 for x in
                          enumerate(data)]
    ...
    def get_num_count(self):
        return sum([1 if not isinstance(x, list) else 0 for x in self.value.reshape(1, -1)[0]])
    ...
    def _get_point_score(self, point):
        ...
        if isinstance(item, list):
            score = 10 - len(item)
            score += sum([1 if isinstance(x, int) else 0 for x in self.value[r]])
            score += sum([1 if isinstance(x, int) else 0 for x in self.value[:, c]])
            return score
        else:
            return 0
